...
import com.quizmaster.databinding.FragmentAddQuizBinding
...
class AddQuizFragment : Fragment() {
    ...
    private fun setupUI() {
        ...
        binding.addQuestionBtn.setOnClickListener { addQuestionView() }
        binding.saveQuizBtn.setOnClickListener { saveQuiz() }
        observeCategories()
        observeSaveStatus()
    }
    ...
    private fun observeSaveStatus() {
        quizViewModel.saveStatus.observe(viewLifecycleOwner) {
    ...
    private fun saveQuiz() {
        ...
        if (title.isNotBlank() && questions.isNotEmpty()) {
            ...
            quizViewModel.saveQuiz(Quiz("", title, category, questions))
        } else {
